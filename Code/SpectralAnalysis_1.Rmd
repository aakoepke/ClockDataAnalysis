...
plot(f,log10(MTSE_fd$spectrum[-1]), type = "l", main = "Full vs. Partial Data MTSE comparison, d = 0.4")
...
plot(f,log10(MTSE_fd$spectrum[-1]), type = "l", main = "Full vs. Partial Data MTSE comparison, d = 0.25", ylab = "log10(S(f))")
lines(f.missing, log10(MTSE_fd_missing$spectrum[-1]), col = "red")
lines(log10(seq(0.001,0.5,length.out = 1025)),-2*d*log10(2*abs(sin(abs(pi*seq(0.005,0.5,length.out = 1025))))), col = "blue")
legend(x = -3, y = 0.5, legend = c("Full Data", "Missing Data", "Truth"), col = c("black", "red", "blue"), lty = 1)
```
...
```{r}
MTSE_fd1 <- multitaper_est(X.t = X.t, NW = 1, K = 1)
MTSE_fd2 <- multitaper_est(X.t = X.t, NW = 2, K = 2)
MTSE_fd3 <- multitaper_est(X.t = X.t, NW = 3, K = 3)
MTSE_fd4 <- multitaper_est(X.t = X.t, NW = 4, K = 4)
...
plot(f,log10(MTSE_fd1$spectrum[-1]), type = "l", main = "NW = 1", ylab = "log10(S(f))")
lines(log10(seq(0.001,0.5,length.out = 1025)),-2*d*log10(2*abs(sin(abs(pi*seq(0.005,0.5,length.out = 1025))))), col = "blue")
...
AVAR_missing <- getAvars(length(na.omit(y)),na.omit(y)) #31903 data points
